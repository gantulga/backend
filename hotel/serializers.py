...
from financial_app.models import Money_transfer, Currency, Money_transfer_type
import json
...
class HotelRoomsSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'number', 'description',
                  'client_products', 'client_items', 'free', 'clean', 'minibarFull', 'client_items']
...
class HotelOrdersSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        for payment in payments_data:
            ...
            print(payment)
            if payment["amount"] != 0:
                payment1 = Payment.objects.create(**payment)
                currency = Currency.objects.get(pk=1)
                money_transfer_type = Money_transfer_type.objects.get(pk=1)
