...
class Order(Modifiedinfo):
	...
	products = models.ManyToManyField('product_app.Product', through='Order_detial')
	payments = models.ManyToManyField('Payment', through='Order_payments')
...
class Order_payments(Modifiedinfo):
	order = models.ForeignKey('Order', on_delete=models.DO_NOTHING, related_name="order_payments")
	payment = models.ForeignKey('Payment', on_delete=models.DO_NOTHING, related_name="payement_orders")

#Захиалгын төлбөр
class Bill(Modifiedinfo):
	client = models.ForeignKey('structure_app.Client', null=False, blank=False, on_delete=models.DO_NOTHING, related_name="client_bills")
	total_amount = models.DecimalField(max_digits=14, decimal_places=2, null=False, blank=False, default=0)
	discount_rate = models.IntegerField(default=0, validators=[MinValueValidator(0), MaxValueValidator(100)])
	discounted_amount = models.DecimalField(max_digits=14, decimal_places=2, null=False, blank=False, default=0)
	amount = models.DecimalField(max_digits=14, decimal_places=2, null=False, blank=False, default=0)
	status = models.CharField(null=False, max_length=255)
	...
	division = models.ForeignKey('structure_app.Division', null=False, blank=False, on_delete=models.DO_NOTHING, related_name="division_bills")
	customer = models.ForeignKey('structure_app.Customer', null=False, blank=False, on_delete=models.DO_NOTHING, related_name="customer_bills")
...
class Payment_bills(Modifiedinfo):
	bill = models.ForeignKey('Bill', on_delete=models.DO_NOTHING, related_name="bill_payments")
	payment = models.ForeignKey('Payment', on_delete=models.DO_NOTHING, related_name="payment_bills")
